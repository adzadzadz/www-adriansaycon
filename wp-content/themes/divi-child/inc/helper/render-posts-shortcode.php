<?php
/**
 * Plugin Name: TCM Posts Display
 * Description: Display post list
 * Version: 1.0.0
 * Author: Adrian Saycon
 * Author URI: https://www.adriansaycon.com
 */

add_shortcode( 'tcm-posts', 'tcm_posts_shortcode' );
/**
 * Callback for the tcm-posts shortcode.
 *
 * @param array $atts Shortcode attributes.
 * @return string
 */
function tcm_posts_shortcode( $atts ) {
	...
	$atts = shortcode_atts(
		array(
			...
			'title'                 => '',
			'wrapper'               => 'ul',
			'wrapper_class'         => 'tcm-posts-listing',
			'wrapper_id'            => false,
		),
		$atts,
		'tcm-posts'
	...
	if ( ! empty( $post_type ) ) {
		$args['post_type'] = tcm_dps_off( $post_type );
	}
	...
	if ( ! empty( $date ) || ! empty( $time ) || ! empty( $date_query_after ) || ! empty( $date_query_before ) ) {
		...
		$dates = tcm_sanitize_datetime( $date );
		if ( ! empty( $dates ) ) {
			if ( is_string( $dates ) ) {
		...
		$times = tcm_sanitize_datetime( $time, 'time' );
		if ( ! empty( $times ) ) {
		...
		$before = tcm_sanitize_datetime( $date_query_before, 'date', true );
		if ( ! empty( $before ) ) {
			$initial_date_query['before'] = $before;
		}

		// Date query 'after' argument.
		$after = tcm_sanitize_datetime( $date_query_after, 'date', true );
		if ( ! empty( $after ) ) {
	...
	// If Post IDs.
	if ( $id ) {
		$posts_in         = array_map( 'intval', tcm_dps_off( $id ) );
	...
	$post__not_in = array();
	if ( ! empty( $exclude ) ) {
		$post__not_in = array_map( 'intval', tcm_dps_off( $exclude ) );
	}
	...
	// Post Status.
	$post_status = tcm_dps_off( $post_status );
	$validated   = array();
	...
	if ( ! empty( $taxonomy ) && ! empty( $tax_term ) ) {
		...
		while (
			...
			$terms                = tcm_dps_off( sanitize_text_field( $original_atts[ 'tax_' . $count . '_term' ] ) );
	...
	if ( false !== $post_parent__in ) {
		$args['post_parent__in'] = array_map( 'intval', tcm_dps_off( $atts['post_parent__in'] ) );
	}
	if ( false !== $post_parent__not_in ) {
		$args['post_parent__not_in'] = array_map( 'intval', tcm_dps_off( $atts['post_parent__in'] ) );
	}
	 ...
	 */
	global $tcm_listing;
	$tcm_listing = new WP_Query( apply_filters( 'display_posts_shortcode_args', $args, $original_atts ) );
	if ( ! $tcm_listing->have_posts() ) {
		/**
	...
	$inner = '';
	while ( $tcm_listing->have_posts() ) :
		$tcm_listing->the_post();
		global $post;
		...
		if ( $include_date ) {
			$date = 'relative' === $date_format ? tcm_dps_relative_date( get_the_date( 'U' ) ) : get_the_date( $date_format );
		} elseif ( $include_date_modified ) {
			$date = 'relative' === $date_format ? tcm_dps_relative_date( get_the_modified_time( 'U' ) ) : get_the_modified_date( $date_format );
		}
		if ( ! empty( $date ) ) {
		...
		if ( $include_content ) {
			add_filter( 'shortcode_atts_tcm-posts', 'tcm_posts_off', 10, 3 );
			/** This filter is documented in wp-includes/post-template.php */
			$content = '<div class="' . implode( ' ', $content_class ) . '">' . apply_filters( 'the_content', get_the_content() ) . '</div>';
			remove_filter( 'shortcode_atts_tcm-posts', 'tcm_posts_off', 10, 3 );
		}
		 ...
		 * @param WP_Query $tcm_listing       WP_Query object for the posts listing.
		 * @param array    $original_atts Original attributes passed to the shortcode.
		 */
		$class  = array_map( 'sanitize_html_class', apply_filters( 'display_posts_shortcode_post_class', $class, $post, $tcm_listing, $original_atts ) );
	 ...
	 * @param array  $original_atts Original attributes passed to the shortcode.
	 * @param object $tcm_listing, WP Query object
	 */
	$open = apply_filters( 'display_posts_shortcode_wrapper_open', '<' . $wrapper . $wrapper_class . $wrapper_id . '>', $original_atts, $tcm_listing );
	 ...
	 * @param object $tcm_listing, WP Query object
	 */
	$close = apply_filters( 'display_posts_shortcode_wrapper_close', '</' . $wrapper . '>', $original_atts, $tcm_listing );
	...
	if ( $shortcode_title ) {
		...
		$return .= '<' . $title_tag . ' class="tcm-posts-title">' . $shortcode_title . '</' . $title_tag . '>' . "\n";
	}
 ...
 *                      an empty array.
 */
function tcm_sanitize_datetime( $date_time, $type = 'date', $accepts_string = false ) {
 ...
 *
 * If display full post content, any uses of [tcm-posts] are disabled.
 *
 ...
 * @return array
 */
function tcm_posts_off( $out, $pairs, $atts ) {
	/**
	 * Filter whether to disable the tcm-posts shortcode.
	 *
	 * The function and filter were added for backward-compatibility with
	 * 2.3 behavior in certain circumstances.
	 *
	 * @param bool $disable Whether to disable the tcm-posts shortcode. Default true.
	 */
 ...
 */
function tcm_dps_relative_date( $date ) {
	return human_time_diff( $date ) . ' ' . __( 'ago', 'tcm-posts' );
}